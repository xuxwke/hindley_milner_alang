...
from ALangLexer import ALangLexer
from ALangParser import ALangParser
from ALangVisitor import ALangVisitor
...
def typeInferInputStream(input_stream: ant.InputStream) -> str:
    ...
    # 并且记录类型在实例变量中
    visitor.visit(tree)
    return visitor.symbolTypeJson()



def typeInferCode(code: str) -> str:
    input_stream = ant.InputStream(code)
    return typeInferInputStream(input_stream)


def typeInferFilePath(filePath: str) -> str:
    import os
